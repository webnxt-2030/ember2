...
async function backfillEscrow(escrowAddress: `0x${string}`) {
  ...
  for (const escrowEvent of escrowEvents) {
    const { name } = escrowEvent;
    ...
    const events = await publicClient.getLogs({
      ...
      logIndex: number;
      args: Parameters<typeof escrowEvent.handler>[0]["args"];
    }[];
...
async function dispatchEvent(
  ...
  log: { blockNumber: bigint; transactionHash: `0x${string}`; logIndex: number; args: unknown }
) {
  switch (eventName) {
    case "Contributed":
