...
export function formatContractError(
  ...
  fallback = "Transaction failed. Please try again or contact support if the problem persists."
): string | null {
  if (!err) return null;
 ...
 */
export function makeFormatContractError(
  fallback?: string
): (err: Error | null | undefined) => string | null {
