 ...
 */
export function computeMilestoneBps(curve, n) {
    if (n < MIN_MILESTONES || n > MAX_MILESTONES) {
        throw new RangeError(`n must be between ${String(MIN_MILESTONES)} and ${String(MAX_MILESTONES)}, got ${String(n)}`);
    }
    switch (curve) {
 ...
 */
function computeBinary(n) {
    const bps = Array(n).fill(0);
    const rest = n - 1;
 ...
 */
export function validateMilestoneBps(bps) {
    if (bps.length < MIN_MILESTONES || bps.length > MAX_MILESTONES)
        return false;
    if (bps.some((v) => !Number.isInteger(v) || v < 0))
        return false;
    return bps.reduce((a, b) => a + b, 0) === MILESTONE_BPS_TOTAL;
}
//# sourceMappingURL=reward-curve.js.map